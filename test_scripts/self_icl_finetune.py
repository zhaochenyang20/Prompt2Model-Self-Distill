...
def get_n_balanced_dataset(dataset, desired_sample_number):
    unique_labels = set(dataset['output_col'])
    label_indices = {label: [] for label in unique_labels}
    for i, label in enumerate(dataset['output_col']):
        label_indices[label].append(i)
    desired_samples_per_label = desired_sample_number // len(unique_labels)
    balanced_indices = []
    for label, indices in label_indices.items():
        balanced_indices.extend(indices[:desired_samples_per_label])
...
def finetune_vicuna(
    ...
    original_dataset = load_from_disk(dataset_path).filter(filter_func)
    # TODO change n
    n = 22

    if exact_match:
        dataset = get_n_balanced_dataset(original_dataset, n)
    else:
        dataset = original_dataset.select(range(n))
...
# TODO 
project_name = 'self_icl_finetune'
# TODO 
task_name = 'task284'
# TODO 
prompt_spec = task_284
# TODO 
training_epochs = 3
# TODO 
is_classification = True
