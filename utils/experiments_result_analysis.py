...
experiement_path = f'/home/cyzhao/finished_experiments/NI_task_020_exp_'
# /home/cyzhao/finished_experiments/SQuAD_experiments_{i}

# dataset_size = {}  # size of filtered dataset
# for i in valid_experiments:
#     dataset_size[i] = []
#     main_folder = experiement_path + f'{i}'
...
#     epoch_2_results[i] = []
#     epoch_3_results[i] = []
#     file_path = experiement_path + f'{i}' + "/experiment_results.csv" 
...
for i in valid_experiments:
    ...
    if os.path.exists(main_folder):
        for sub_folder in os.listdir(main_folder):
            sub_folder_path = os.path.join(main_folder, sub_folder)
            if os.path.isdir(sub_folder_path):
                generated_contents_path = os.path.join(sub_folder_path, "generated_contents")
                if os.path.exists(generated_contents_path):
                    generated_outputs = []
                    for sub_generated_conent_path in os.listdir(generated_contents_path):
                        dataset = datasets.load_from_disk(os.path.join(generated_contents_path,sub_generated_conent_path))
                        generated_outputs.append(dataset)
                    common_inputs_count = sum(1 for i, j, k in zip(generated_outputs[0], generated_outputs[1], generated_outputs[2]) if i == j == k)
                    total_inputs = len(generated_outputs[0])
                    common_inputs_percentage = common_inputs_count / total_inputs
                    print(common_inputs_percentage)
                else:
                    print(f"Dataset file not found in: {sub_folder_path}")
    else:
        print(f"Main folder not found: {main_folder}")